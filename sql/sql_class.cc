...
#ifdef WITH_WSREP
  mysql_mutex_init(key_LOCK_wsrep_data, &LOCK_wsrep_data, MY_MUTEX_INIT_FAST);
  mysql_mutex_record_order(&LOCK_thd_data, &LOCK_wsrep_data); // MDEV-23328
...
#ifdef WITH_WSREP
  delete wsrep_rgi;
  mysql_mutex_destroy(&LOCK_wsrep_data);
#endif
